...
python -u train_DisCoLmi.py 10 full fitzpatrick DisCoLmi
python -u train_DisCoLmi.py 10 full ddi DisCoLmi
'''
...
def train_model(label, dataloaders, device, dataset_sizes, model,
                criterion, optimizer, scheduler, num_epochs=2, alpha=1.0, beta=0.8, lam_mi=1.0):
    print('hyper-parameters alpha: {}  beta: {} lam_mi: {}'.format(alpha, beta, lam_mi))
    since = time.time()
    training_results = []
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for n_iter, batch in enumerate(dataloaders[phase]):
                ...
                inputs_D = batch.get("image_D", inputs).to(device)
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        inputs_D = inputs_D.float()
                        output_D = model(inputs_D)
                        loss_mi = F.mse_loss(output[3], output_D[3])
                    else:
                        loss_mi = torch.tensor(0.0).to(device)
                        
                    loss = loss0+loss1*alpha+loss2+loss3*beta + loss_mi*lam_mi
                ...
                if phase == 'train':
                    ...
                    writer.add_scalar('Loss/'+phase+'loss_mi', loss_mi.item(), train_step)
...
class SkinDataset():
    def __init__(self, dataset_name, csv_file, root_dir, transform=None, transform_D=None):
        """
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
            transform_D (callable, optional): Heavy augmentation transform for pseudo-modality
        """
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image_c = self.transform(image)
        else:
            image_c = image
            
        if hasattr(self, 'transform_D') and self.transform_D is not None:
            image_D = self.transform_D(image)
        else:
            image_D = image_c
...
def custom_load(
    ...
    transform_D_compose = transforms.Compose([
    ...
    transformed_train = SkinDataset(
        ...
        transform_D=transform_D_compose
        )
    ...
    for holdout_set in ["random_holdout"]: # ["expert_select","random_holdout", "a12", "a34","a56", "dermaamin","br"]:
        ...
        for indexer, label in enumerate([level]):
            ...
            model_ft, training_results = train_model(
                ...
                lam_mi=1.0) # Using Modality Invariance 
